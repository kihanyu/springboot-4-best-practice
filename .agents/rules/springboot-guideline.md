...
## 8. 테스트 (Testing)
- **프레임워크**: **JUnit 6 및 JUnit Platform 최신 엔진 기반**의 테스트 구축을 의무화합니다.
- **JUnit 6 기반 핵심 권장사항**:
  - **가상 스레드 병렬 테스트**: 대규모 테스트 스케줄링 시 Java 25 가상 스레드 병렬 실행 기능을 조화롭게 수용하여 속도를 최적화합니다.
  - **불변 단정문 사용**: AssertJ의 `assertThat` 계열 유체(Fluent) API를 활용해 자바 25 레코드 및 데이터 불변 검증을 가독성 있게 처리합니다.
- **단위 테스트 (Unit Test)**: 
  - 스프링 부트스트랩 비용이 없는 **순수 JUnit 6 & Mockito 단위 테스트** 작성을 1순위로 지향합니다.
  - `@ExtendWith(MockitoExtension.class)` 또는 `Mockito.mock()`를 직접 사용하여 0.01초 만에 실행이 끝나는 구조를 유지합니다.
- **통합/웹 계층 테스트 (Integration Test)**:
  - 필요 시에만 `@SpringBootTest`를 활용하되, 스프링 부트 4 환경 규격에 맞춰 구형 `@MockBean` 대신 반드시 **`@MockitoBean`**을 전면 활용하여 모킹합니다.
